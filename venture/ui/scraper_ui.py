# ui/scraper_ui.py - UPDATED FOR THE VERGE
...
class ScraperApp:
    def __init__(self, root):
        self.root = root
        self.root.title("The Verge Author Detail Scraper")
    ...
    def build_ui(self):
        ...
        title_frame = tk.Frame(row1, bg="#00D474")  # The Verge Green
        title_frame.pack(side="left", padx=(0, 20), pady=3)
        
        title_label = tk.Label(
            title_frame,
            text="📰 The Verge Author Detail Scraper",
            font=("Arial", 14, "bold"),
            fg="#1A1A1A",  # Dark text
            bg="#00D474",  # The Verge Green
            padx=10,
            pady=5
        )
        title_label.pack()
        ...
        self.sheet_name_var = tk.StringVar(value="theverge-authors")
    ...
    def initialize_categories(self):
        """Initialize The Verge categories with the provided URLs"""
        self.categories = {
            "Tech": [
                "tech", "amazon", "apple", "facebook", 
                "google", "microsoft", "samsung", "business"
            ],
            "Reviews": [
                "smart-home-review", "phone-review", "tablet-review", 
                "headphone-review", "reviews"
            ],
            "Science": [
                "space", "energy", "environment", "health", "science"
            ],
            "Policy": [
                "policy", "antitrust", "politics", "law", "cyber-security"
            ],
            "Transportation": [
                "transportation", "electric-cars", "autonomous-cars", "ride-sharing"
            ]
        }
    ...
    def on_main_category_selected(self, event=None):
        ...
        for i, subcat in enumerate(subcategories):
            ...
            if "-" in subcat:
                # For hyphenated names like "smart-home-review", convert to "Smart Home Review"
...
10. Save settings"""
        ...
        def reset_settings():
            ...
            if result:
                try:
                    ...
                    self.sheet_name_var.set("theverge-authors")
                    self.json_file_var.set("")
                    self.sheet_name_entry_var.set("theverge-authors")
    ...
    def scrape_articles(self):
        ...
        # Get categories from config manager (which now has Verge URLs)
    ...
    def preview_data(self):
        ...
        if self.authors:
            ...
            for i, author in enumerate(self.authors[:50], 1):
                name = author.get("name", author.get("Author Name", "No Name"))
