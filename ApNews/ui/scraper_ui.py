# ui/scraper_ui.py - UPDATED FOR AP NEWS
...
# Import modules
from modules import ap_scraper_core, enhancer_core, sheets_manager, config_manager
...
class ScraperApp:
    def __init__(self, root):
        self.root = root
        self.root.title("AP News Author Detail Scraper")
    ...
    def build_ui(self):
        ...
        self.threads_var = tk.IntVar(value=10)  # AP News can handle more threads
        
        # Create a frame for the title with background
        title_frame = tk.Frame(row1, bg="#0057B7")  # AP News Blue
        title_frame.pack(side="left", padx=(0, 20), pady=3)
        
        title_label = tk.Label(
            title_frame,
            text="📰 AP News Author Detail Scraper",
            font=("Arial", 14, "bold"),
            fg="white",  # White text
            bg="#0057B7",  # AP News Blue
            padx=10,
            pady=5
        )
        title_label.pack()
        
        # Add threads entry only (no pages for AP News)
        ...
        self.sheet_name_var = tk.StringVar(value="apnews-authors")
    ...
    def initialize_categories(self):
        """Initialize AP News categories with the provided URLs"""
        self.categories = {
            "World News": [
                "world-news"
            ],
    ...
    def on_main_category_selected(self, event=None):
        ...
        for i, subcat in enumerate(subcategories):
            ...
            if subcat == "world-news":
                label = "World News"
            elif subcat == "us-news":
                label = "US News"
            elif subcat == "politics":
                label = "Politics"
            elif subcat == "sports":
                label = "Sports"
            elif subcat == "entertainment":
                label = "Entertainment"
            elif subcat == "business":
                label = "Business"
            elif subcat == "science":
                label = "Science"
            else:
...
10. Save settings"""
        ...
        def reset_settings():
            ...
            if result:
                try:
                    ...
                    self.sheet_name_var.set("apnews-authors")
                    self.json_file_var.set("")
                    self.sheet_name_entry_var.set("apnews-authors")
    ...
    def scrape_articles(self):
        ...
        for subcat in selected_categories:
            if subcat in all_categories:
                ...
                # Use the AP News scraper function
                from modules import ap_scraper_core
                articles = ap_scraper_core.scrape_articles(url, self.log)
                
                # Add category info to articles
                for article in articles:
                    article["category"] = subcat
    ...
    def scrape_authors(self):
        ...
        # Use the AP News scraper's author extraction
        self.authors = ap_scraper_core.scrape_authors_from_articles(
            self.articles,
    ...
    def launch_email_tool(self):
        ...
        try:
            ...
            if getattr(sys, 'frozen', False):
                ...
                project_root = os.path.dirname(current_dir)  # apnews-outreach folder
    ...
    def export_csv(self):
        ...
        try:
            ...
            filename = f"apnews_enhanced_authors_{timestamp}.csv"
    ...
    def preview_data(self):
        ...
        if self.authors:
            ...
            for i, author in enumerate(self.authors[:50], 1):
                ...
                title = author.get("Job Title", "No Title")
                email = author.get("Email", "No Email")
                authors_text.insert("end", f"{i}. {name} | {title[:30]} | Email: {email[:20]}...\n")
