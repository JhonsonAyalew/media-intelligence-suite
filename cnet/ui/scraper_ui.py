# ui/scraper_ui.py - UPDATED FOR CNET
...
# Import modules
from modules import cnet_scraper_core, enhancer_core, sheets_manager, config_manager
...
class ScraperApp:
    def __init__(self, root):
        self.root = root
        self.root.title("CNET Author Detail Scraper")
    ...
    def build_ui(self):
        ...
        self.pages_var = tk.IntVar(value=1)  # Default to 1 page for CNET
        self.threads_var = tk.IntVar(value=10)  # CNET can handle more threads
        
        # Create a frame for the title with background
        title_frame = tk.Frame(row1, bg="#E31837")  # CNET Red
        title_frame.pack(side="left", padx=(0, 20), pady=3)
        
        title_label = tk.Label(
            title_frame,
            text="📰 CNET Author Detail Scraper",
            font=("Arial", 14, "bold"),
            fg="white",  # White text
            bg="#E31837",  # CNET Red
            padx=10,
        ...
        self.sheet_name_var = tk.StringVar(value="cnet-authors")
    ...
    def initialize_categories(self):
        """Initialize CNET categories with the provided URLs"""
        self.categories = {
            "AI & Tech": [
                "ai-atlas", "tech"
            ],
    ...
    def on_main_category_selected(self, event=None):
        ...
        for i, subcat in enumerate(subcategories):
            ...
            # Create readable label for checkbox
            if subcat == "ai-atlas":
                label = "AI Atlas"
            elif subcat == "tech":
                label = "Tech"
            ...
            # Create checkbutton with readable label
            cb = ttk.Checkbutton(self.subcat_frame, text=label, 
...
10. Save settings"""
        ...
        def reset_settings():
            ...
            if result:
                try:
                    ...
                    self.sheet_name_var.set("cnet-authors")
                    self.json_file_var.set("")
                    self.sheet_name_entry_var.set("cnet-authors")
    ...
    def scrape_articles(self):
        ...
        # For CNET, we just need the pages count - categories are handled by URL structure
        pages = self.pages_var.get()
        
        # Use the CNET scraper's direct function
        self.log(f"Scraping {pages} pages from CNET news...", "progress")
        
        # Use the new CNET scraper function
        from modules import cnet_scraper_core
        articles = cnet_scraper_core.scrape_articles(pages, self.log)
        
        # Add category info to articles (just for reference)
        for article in articles:
            article["category"] = ", ".join(selected_categories)
    ...
    def scrape_authors(self):
        ...
        # Use the CNET scraper's author extraction
        self.authors = cnet_scraper_core.scrape_authors_from_articles(
            self.articles,
    ...
    def launch_email_tool(self):
        ...
        try:
            ...
            if getattr(sys, 'frozen', False):
                ...
                project_root = os.path.dirname(current_dir)  # cnet-outreach folder
    ...
    def export_csv(self):
        ...
        try:
            ...
            filename = f"cnet_enhanced_authors_{timestamp}.csv"
    ...
    def preview_data(self):
        ...
        if self.authors:
            ...
            for i, author in enumerate(self.authors[:50], 1):
                name = author.get("Author Name", author.get("name", "No Name"))
                twitter = author.get("Twitter", "No Twitter")
                authors_text.insert("end", f"{i}. {name} | Twitter: {twitter[:30]}...\n")
