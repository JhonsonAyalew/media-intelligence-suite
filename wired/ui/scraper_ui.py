# ui/scraper_ui.py - UPDATED FOR WIRED
...
# Import modules
from modules import wired_scraper_core, enhancer_core, sheets_manager, config_manager
...
class ScraperApp:
    def __init__(self, root):
        self.root = root
        self.root.title("WIRED Author Detail Scraper")
    ...
    def build_ui(self):
        ...
        self.threads_var = tk.IntVar(value=8)  # Only threads input
        
        # Create a frame for the title with background
        title_frame = tk.Frame(row1, bg="#000000")  # WIRED Black
        title_frame.pack(side="left", padx=(0, 20), pady=3)
        
        title_label = tk.Label(
            title_frame,
            text="📰 WIRED Author Detail Scraper",
            font=("Arial", 14, "bold"),
            fg="#FFFFFF",  # White text
            bg="#000000",  # WIRED Black
            padx=10,
            pady=5
        )
        title_label.pack()
        
        # Add threads entry (pages removed for WIRED)
        ...
        self.sheet_name_var = tk.StringVar(value="wired-authors")
    ...
    def initialize_categories(self):
        """Initialize WIRED categories with the provided URLs"""
        self.categories = {
            "Main": [
                "home"
            ],
    ...
    def on_main_category_selected(self, event=None):
        ...
        for i, subcat in enumerate(subcategories):
            ...
            if subcat == "home":
                label = "WIRED Home"
            elif subcat == "security":
                label = "Security"
            elif subcat == "politics":
                label = "Politics"
            elif subcat == "big-story":
                label = "Big Story"
            elif subcat == "science":
                label = "Science"
            elif subcat == "culture":
...
10. Save settings"""
        ...
        def reset_settings():
            ...
            if result:
                try:
                    ...
                    self.sheet_name_var.set("wired-authors")
                    self.json_file_var.set("")
                    self.sheet_name_entry_var.set("wired-authors")
    ...
    def scrape_articles(self):
        ...
        for subcat in selected_categories:
            if subcat in all_categories:
                ...
                # For WIRED, we don't pass pages parameter
                articles = wired_scraper_core.scrape_articles(
                    url, 
                    1,  # WIRED only needs 1 page
                    self.log
                )
    ...
    def scrape_authors(self):
        ...
        self.authors = wired_scraper_core.scrape_authors_from_articles(
            self.articles,
    ...
    def launch_email_tool(self):
        ...
        try:
            ...
            if getattr(sys, 'frozen', False):
                ...
                project_root = os.path.dirname(current_dir)  # wired-outreach folder
    ...
    def export_csv(self):
        ...
        try:
            ...
            filename = f"wired_enhanced_authors_{timestamp}.csv"
    ...
    def preview_data(self):
        ...
        if self.authors:
            ...
            for i, author in enumerate(self.authors[:50], 1):
                ...
                title = author.get("Title", "No Title")[:30]
                authors_text.insert("end", f"{i}. {name} | {title}\n")
