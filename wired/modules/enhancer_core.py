# modules/enhancer_core.py - SIMPLIFIED VERSION
import pandas as pd
import re
...
def deduplicate_authors(authors: List[Dict]) -> List[Dict]:
    ...
    for author in authors:
        ...
        if not url:
            # Also check by name if no URL
            name = author.get("Author Name", "").strip().lower()
            if name:
        ...
        if key in authors_by_url:
            # Merge article counts if same author
            existing = authors_by_url[key]
            ...
            # Keep Twitter if available
            if not existing.get("Twitter Handle") and author.get("Twitter Handle"):
                existing["Twitter Handle"] = author["Twitter Handle"]
            if not existing.get("Twitter URL") and author.get("Twitter URL"):
                existing["Twitter URL"] = author["Twitter URL"]
            
            # Keep LinkedIn if available
            if not existing.get("LinkedIn Handle") and author.get("LinkedIn Handle"):
                existing["LinkedIn Handle"] = author["LinkedIn Handle"]
            if not existing.get("LinkedIn URL") and author.get("LinkedIn URL"):
                existing["LinkedIn URL"] = author["LinkedIn URL"]
                
        else:
            authors_by_url[key] = author.copy()
...
def is_valid_twitter_handle(handle: str) -> bool:
    ...
    if handle.lower() in ["", "unknown", "n/a", "none", "no twitter"]:
        return False
    
    # Check if it's a valid Twitter handle format
    return bool(re.match(r'^@?[a-zA-Z0-9_]{1,15}$', handle.replace('@', '')))
...
def is_valid_linkedin_url(url: str) -> bool:
    ...
    # Skip empty or placeholder values
    if url.lower() in ["", "unknown", "n/a", "none", "no linkedin"]:
        return False
...
def clean_social_media_data(author: Dict) -> Dict:
    ...
    if twitter_handle:
        ...
        cleaned["Twitter Handle"] = twitter_handle
    
    # Clean Twitter URL
    twitter_url = author.get("Twitter URL", "")
    if twitter_url:
        twitter_url = twitter_url.strip()
        # Ensure it's a proper URL
        if twitter_url and not twitter_url.startswith('http'):
            twitter_url = f"https://twitter.com/{twitter_url.replace('@', '')}"
        cleaned["Twitter URL"] = twitter_url
    
    # Clean LinkedIn URL
    linkedin_url = author.get("LinkedIn URL", "")
    if linkedin_url:
        linkedin_url = linkedin_url.strip()
        # Ensure it's a proper URL
        if linkedin_url and not linkedin_url.startswith('http'):
            linkedin_url = f"https://linkedin.com/in/{linkedin_url}"
        cleaned["LinkedIn URL"] = linkedin_url
    
    # Clean LinkedIn Handle
    linkedin_handle = author.get("LinkedIn Handle", "")
    if linkedin_handle:
        linkedin_handle = linkedin_handle.strip()
        cleaned["LinkedIn Handle"] = linkedin_handle
...
def clean_author_data(author: Dict) -> Dict:
    ...
    cleaned = clean_social_media_data(cleaned)
...
def filter_valid_authors(authors: List[Dict]) -> List[Dict]:
    """Filter authors to keep only those with valid profiles and social media"""
    valid_authors = []
    ...
    for author in authors:
        ...
        # Check profile URL
        profile_url = author.get("Profile URL", "")
        if not profile_url or profile_url.lower() in ["", "unknown", "n/a", "none"]:
            continue
        
        # Check if author has any contact info (email or social media)
        has_email = bool(author.get("Email", "").strip())
        has_twitter = bool(author.get("Twitter Handle", "").strip()) or bool(author.get("Twitter URL", "").strip())
        has_linkedin = bool(author.get("LinkedIn Handle", "").strip()) or bool(author.get("LinkedIn URL", "").strip())
        has_contact_info = bool(author.get("Contact Info", "").strip())
        
        # Keep author if they have at least one contact method
        if has_email or has_twitter or has_linkedin or has_contact_info:
...
def apply_enhancements(articles: List[Dict], authors: List[Dict]) -> Tuple[List[Dict], List[Dict], List[Dict]]:
    ...
    4. Clean author data (extract emails, clean social media)
    5. Filter valid authors (must have contact info)
    6. Add article frequency data
    7. Assign primary topics
    
    Returns: (cleaned_articles, cleaned_authors, valid_authors)
    """
    ...
    # 7️⃣ Filter for authors with valid contact info
    valid_authors = filter_valid_authors(cleaned_authors)
    
    # Return 3 values to match what the UI expects
    return articles, cleaned_authors, valid_authors


def validate_social_media_counts(authors: List[Dict]) -> Dict:
    """Count how many authors have social media profiles"""
    counts = {
    ...
    for author in authors:
        ...
        has_linkedin = bool(author.get("LinkedIn Handle", "").strip()) or bool(author.get("LinkedIn URL", "").strip())
        has_email = bool(author.get("Email", "").strip())
        has_contact = bool(author.get("Contact Info", "").strip())
        
        if has_twitter:
            counts["with_twitter"] += 1
        if has_linkedin:
            counts["with_linkedin"] += 1
        if has_email:
            counts["with_email"] += 1
        if has_twitter or has_linkedin or has_email or has_contact:
            counts["with_any_contact"] += 1
    
    # Calculate percentages
    if counts["total_authors"] > 0:
        counts["twitter_percentage"] = (counts["with_twitter"] / counts["total_authors"]) * 100
        counts["linkedin_percentage"] = (counts["with_linkedin"] / counts["total_authors"]) * 100
        counts["email_percentage"] = (counts["with_email"] / counts["total_authors"]) * 100
        counts["any_contact_percentage"] = (counts["with_any_contact"] / counts["total_authors"]) * 100
    else:
        counts.update({
